...
import fetchNearbyAmenities from '../fetch/fetchplaces';
...
const AmenitiesMarker = ({ toggleAmenity, selectedAmenity, location, isNearest, staticOnly = false }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (!staticOnly) {
                ...
                fetchedAmenities = amenities_static.map((amenity) => {
                    const distance = calculateDistance(location, amenity);
                    const distanceColor = getDistanceColor(distance);
                    return { ...amenity, distance: distance.toFixed(2), distanceColor };
                });
            }
...
export default AmenitiesMarker;